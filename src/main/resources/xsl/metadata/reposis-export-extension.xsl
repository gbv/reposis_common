...
<xsl:stylesheet version="1.0"
  ...
  <xsl:import href="xslImport:modsmeta:metadata/reposis-export-extension.xsl"/>

  <xsl:template match="/">
    <xsl:variable name="objectId" select="/mycoreobject/@ID"/>
    <xsl:if test="$mods-type='software'">
      <div id="reposis-export-extension">
        ...
        <script>
          $(document).ready(function() {
            ...
            const extensionDiv = $("div#reposis-export-extension-div");
