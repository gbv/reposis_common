...
<xsl:stylesheet version="3.0"
  ...
  <xsl:param name="email-general" select="mcrproperty:one('REP.ErrorPage.Mail.General')" />
  <xsl:param name="email-technical" select="mcrproperty:one('REP.ErrorPage.Mail.Technical')" />
  ...
  <xsl:template match="/mcr_error[@HttpError='500']" mode="error-content">
    ...
    <p>
      ...
      <xsl:if test="string-length($email-technical) &gt; 0">
        Gern können Sie uns eine Mail an
        <xsl:call-template name="mail">
          <xsl:with-param name="address" select="$email-technical" />
  ...
  <xsl:template match="/mcr_error[@HttpError='404']" mode="error-content">
    ...
    <p>
      ...
      <xsl:if test="string-length($email-general) &gt; 0">
        Alternativ können Sie eine Mail an
        <xsl:call-template name="mail">
          <xsl:with-param name="address" select="$email-general" />
